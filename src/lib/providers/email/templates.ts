...
export interface OutreachTemplateData {
  ...
  /** Absender-Profil des Nutzers — ersetzt hardcoded Signatur */
...
function buildSignatureText(sender: SenderProfile): string {
  return `${sender.name}
${sender.title}
...
function buildSignatureHtml(sender: SenderProfile): string {
  return `
<table style="margin-top: 24px; border-top: 2px solid #1F3D2E; padding-top: 0; width: 100%; border-collapse: collapse;">
  <tr>
    <td style="font-size: 14px; color: #222; line-height: 1.8; padding-bottom: 16px;">
      ...
      E-Mail: <a href="mailto:${sender.email}" style="color: #1F3D2E;">${sender.email}</a><br>
...
function htmlWrap(greeting: string, bodyHtml: string, sender: SenderProfile): string {
  return `<!DOCTYPE html>
<html lang="de">
<head>
  <meta charset="UTF-8">
...
<body style="font-family: Arial, sans-serif; font-size: 15px; color: #222; max-width: 600px; margin: 0 auto; padding: 24px; line-height: 1.7;">
  <p>${greeting}</p>
  ${bodyHtml}
  <p style="margin-top: 24px;">Mit freundlichen Grüßen<br><br>${sender.name}</p>
  ${buildSignatureHtml(sender)}
...
function generateErstkontakt(data: OutreachTemplateData): { subject: string; text: string; html: string } {
  ...
  const sender = data.senderProfile ?? DEFAULT_SENDER;
  ...
  const sigText = buildSignatureText(sender);
  const text = `${greeting}
...
Wenn Sie keine weiteren E-Mails von uns erhalten möchten, antworten Sie bitte mit „Abmelden".`;
  ...
  const html = htmlWrap(greeting, `
    <p>mein Name ist ${sender.name}, ich bin ${sender.title} der <strong>GreenScout e.V.</strong> und über einen unserer Mitglieder bin ich auf Ihre Dachfläche aufmerksam gemacht worden.</p>
